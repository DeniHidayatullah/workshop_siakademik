...
<a class='pull-right btn btn-primary btn-sm' href='index.php?view=matapelajaran&act=tambah'>
    Tambahkan Data</a>
<?php } ?>
</div>
</div class="box-body">
...
if($_SESSION[level]!'kepala'){
    echo "<td><center>
    <a class='btn btn-primary btn-xs' title='Detail Data' href='?view=matapelajaran&act=detail&id=$r[kode_pelajaran]'> <span class='glyphicon'
    <a class='btn btn-success btn-xs' title='Edit Data' href='?view=matapelajaran&act=edit&id=$r[kode_pelajaran]'><span class='glyphicon'
    <a class='btn btn-success btn-xs' title='Delete Data' href='?view=matapelajaran&act=hapus&id=$r[kode_pelajaran]'><span class='glyphicon glyphicon'
</center></td>";
}
echo "</tr>";
$no++;
...
<tr><th scope='row>Kode Pelajaran</th>          <td><input type='text' class='form control' name='a' value='$s[kode_pelajaran]"> </td>
<tr><th scope='row'>Nama Mapel</th>          <td><input type='text' class='form control' name='f' value='$s[namamatapelajaran]"> </td>
<tr><th scope='row'>Nama Mapel En</th>          <td><input type='text' class='form control' name='g' value='$s[namamatapelajaran_en]"> </td>
<tr><th scope='row'>Jurusan</th> <td><select classs='form-control' name='c'>
<option value='0' selected>- Pilih Jurusan -</option>";
$jurusan = mysqli_query($koneksi,"SELECTED * FROM jurusan");
while($a=mysqli_fetch_array($jurusan)){
    ...
    while($a = mysqli_fetch_array($guru)){
        ...
        <tr><th scope='row'>Kompetensi Khusus</th>     
        <td><input type='text' class='form-control' name='j' value='$s[kompetensi_khusus]'></td></tr>
        ...
        if (isset($_POST[tambah])){
            ...
            '$_POST[g]','$_POST[h]','$_POST[i]','$_POST[j]','$_POST[k]','$_POST[l]','$_POST[m]')");
            echo "<script>document.location='index.php?view=matapelajaran';</script>";
        }
        ...
        <tr><th scope='row'>Kode Pelajar</th> <td><input type='text' class='form-control' name='a' value='$s[kode_pelajaran]'> </td></tr>
        <tr><th scope='row'>Nama Mapel</th> <td><input type='text' class='form-control' name='f' value='$s[namamatapelajaran]'> </td></tr>
        <tr><th scope='row'>Nama Mapel En</th> 
        <td><input type='text' class='form-control' name='g' value='$s[namamatapelajaran_en]'> </td></tr>
        <tr><th scope='row'>Jurusan</th> <td>select class='form-control' name='name='c'>
        <option value='0' selected>- Pilih Jurusan -</option>";
        ...
        <option value='0' selected>- Pilih Guru Pengampu -</option>";
    $guru = mysqli_query($koneksi,"SELECT * FROM guru");
    while($a = mysqli_fetch_query($guru)){
    echo "<option value='$a[nip]'>$a[nama_guru]</option>";
    }
    echo "</select>
    </td></tr>
    ...
    <tr><th scope='row'>Kelompok</th>               <td><input type='text' class='form-control' name='b'</td></tr>
    <option value='0' selected>- Pilih Kelompok Mata Pelajaran -</option>";
    $kelompok = mysqli_query($koneksi,"SELECT * FROM kelompok_mata_pelajaran");
    while($a = mysqli_fetch_array($kelompok)){
    echo "<option value='$a[id_kelompok_mata_pelajaran]'>$a[nama_kelompok_mata_pelajaran]</option>";
    ...
    $edit = mysqli_query($koneksi,"SELECT a.*, b.nama_kelompok_mata_pelajaran, c.nama_guru
    d.nama_kurikulum,e.nama_jurusan FROM mata_pelajaran a
