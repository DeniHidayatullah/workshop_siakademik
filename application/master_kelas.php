...
<a class='pull_right btn btn_primary btn-sm' href='index.php?view=kelas&tambah'>Tambahkan Data</a>
<?php} ?>
</div><!-- /.box-header -->
<div class="box-body">
...
<th style='widht:70px'>Action</th> 
<?php } ?>
<?tr>
</thead>
<tbody>
<?php
$tampil = mysqli_query($koneksi,"SELECT * FROM kelas a LEFT JOIN guru b ON a.nip=b.nop")
LEFT JOIN jurusan c ON a.kode_jurusan=c.kode_jurusan
LEFT JOIN ruangan d ON a.kode_ruangan=d.kode_ruangan
LEFT JOIN gedung e ON a.kode_gedung=e.kode_gedung
...
echo "<td><center>
<a class='btn btn-succes btn-xs' title='Edit Data' href='?view=kelas&act=edit&id=$r[kode_kelas]'>
...
<div class='box-body'>
<from method='POST' class='form-horizontal' action='' enctype='multipart/form-data'>
<div class='col-md-12'>
<table class='table table-condensed table-bordered'>
</tbody>
<input type='hidden' name='id' value='$s[kode_kelas]'>
<tr><th widht='120px' scope='row'>Kode Kelas</th>
...
echo "</select></td></tr>
<tr><th scope='row'>Ruangan</th>    <td><select class=''form-control' nama='d'>
...
if ($a[kode_jurusan] == $s[kode_ruangan]){
...
echo "</select></td></tr>
<tr><th scope='row'>Nama Kelas</th>
<td><input type='text' class='form-control' name='e' value='$s[nama_kelas]'></td></tr>
...
<table class='table table-condensed table-bordered'>
<tbody>
<tr><th widht='120px' scope='row'>Kode Kelas</th> <td><input type='text' class='form-control' name='a'> </td></tr>
<tr><th scope='row'>Wali Kelas</th> <td><select class='form-control' name='a'> </td></tr>
 <option value='0' selected>- Pilih Wali Kelas -</option>";
 $wali = mysqli_query($koneksi,"SELECT * FROM guru");
 while ($a = mysqli_fetch_array($wali)) {
     echo"<option value='$a[nip]'>$a[nama_guru]</option>";
 ...
 <option value='0' selected>- Pilih Jurusan -</option>";
 $jur = mysqli_query($koneksi, "SELECT * FROM jurusan");
 while ($a = mysqli_fecth_array($jur)) {
 ...
 echo "</select></td></tr>
 <tr><th scope='row'>Nama Kelas</th>   <td><input type='text' class='form-control' name='e' value='$a[nama_kelas]'></td></tr>
